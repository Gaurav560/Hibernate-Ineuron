...
//for select(some columns and rows only ) operation in hibernate no transaction is required
...

		{
			...
			Query<Object[]> query = session
					.createQuery("SELECT price,qty FROM in.ineuron.model.products WHERE pname IN(:prod1,:prod2)");

			// SQL query for this HQL query will be ->select price,qty from products where pname
			// in("savlon","denver");
